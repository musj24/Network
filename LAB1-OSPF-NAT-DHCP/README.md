...
3. 고려할 점
- HQ와 ISP를 연결하는 G0/2에 PAT를 설정하되, Branch 내부서버는 별도의 static NAT 주소를 설정한다. (내부 서버의 변환될 ip는 203.1.113.1 로 한다)

- ISP와 인터넷에 연결되는 인터페이스엔 내부 정보를 전달하지 않기위해, passive-interface를 설정한다.
- 또한 DHCP 서버와 연결된 인터페이스에도 마찬가지로 passive-interface를 설정한다.
- 내부와 Internet을 연결하기 위해, HQ에서 default route를 전파한다. (Internet은 임시로 루프백 주소인 8.8.8.8로 대체한다)
...
4. 검증할 점
- OSPF 및 DHCP 검증을 위해 HQ 내부 와 Branch 내부 PC간 PING 연결 및 내부 pc들의 Internet 연결
- ISP의 G0/0과 G0/1이 passive-interface 인지
- PAT 변환이 HQ의 G0/2 주소로 되는지
- Branch 내부 서버가 제대로 static 변환이 되는지 


5. 트러블슈팅
- default route를 전파하지 않으면?
